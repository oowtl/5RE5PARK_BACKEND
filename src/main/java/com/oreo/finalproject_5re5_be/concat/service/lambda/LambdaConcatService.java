...
public class LambdaConcatService {
    ...
    //결과 오디오 DB 저장
    //병합 결과 반환
    public List<ConcatResultDto> concatOnLambda(ConcatTabResponseDto concatTabResponseDto, ConcatRowRequestDto audioRequests) {
        LambdaConcatRequest lambdaConcatRequest = LambdaConcatRequest.builder()
    ...
    private static List<ConcatResultDto> getConcatResultDtos(List<ConcatResult> concatResults) {
        return concatResults.stream().map(concatResult ->
    ...
    private List<LambdaConcatResultDto> lambdaConcatRequest(LambdaConcatRequest lambdaConcatRequest) {
        Object response = restTemplate.postForObject(
                "https://bewpsh81xa.execute-api.ap-northeast-2.amazonaws.com/default/Test", // Lambda URL
                lambdaConcatRequest,
                Object.class
        );
...
// ObjectMapper를 사용해 JSON을 List<LambdaConcatResultDto>로 변환
    ...
    private List<ConcatResult> getConcatResults(ConcatTabResponseDto concatTabResponseDto, List<LambdaConcatResultDto> list) {
        //재료 오디오와 결과 저장
        /*
        ...
        List<List<MaterialAudio>> materials = list.stream().map(result -> {
            ...
            return processed.stream().map(process -> MaterialAudio.builder()
                    .concatResult(concatResult)
                    .audioFile(AudioFile.builder().audioFileSeq(process.getSeq()).build())
                    .method("NORMAL").build()).toList();
    ...
    public ConcatResult of(LambdaConcatResultDto concatResult, ConcatTabResponseDto concatTabResponseDto) {
        ConcatResult build = ConcatResult.builder()
                .concatTab(ConcatTab
                        .builder()
                        .projectId(concatTabResponseDto.getTabId())
                        .build())
