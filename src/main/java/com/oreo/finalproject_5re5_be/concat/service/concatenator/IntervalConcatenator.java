...
public interface IntervalConcatenator extends Concatenator {
    ...
    static List<AudioInputStream> prepareAudioStreams(List<AudioProperties> audioStreams, AudioFormat audioFormat) {
        ...
        for (AudioProperties audioProperties : audioStreams) {
            AudioInputStream audioStream = audioProperties.audioInputStream(); // AudioProperties에서 AudioStream 추출

            // row audio 관련 log
            System.out.printf("[prepareAudioStreams] 오디오 스트림 추가: index=%d, frameLength=%d, format=%s%n",
                    index, audioStream.getFrameLength(), audioStream.getFormat());

            result.add(audioProperties.audioInputStream());//오디오 스트림 추가

            // 무음 구간 생성 및 추가
            AudioInputStream silenceStream = BeepMaker.makeSound(audioProperties.silence() * 1000, audioFormat);

            // row 무음구간 관련 log
            System.out.printf("[prepareAudioStreams] 무음 구간 추가: index=%d, duration=%dms, frameLength=%d%n",
