...
public class MemberValidator implements Validator {
    ...
    public void validate(Object target, Errors errors) {
        // 타입 변환
        var userRegisterRequest = (MemberRegisterRequest) target;
    ...
    private void checkValidFields(MemberRegisterRequest memberRegisterRequest, Errors errors) {
        // 아이디 유효성 검증
        if (!isValidId(memberRegisterRequest.getId())) {
            errors.rejectValue("id", "invalidId", "아이디는 6~20자의 영문 및 숫자만 허용됩니다.");
        }
        // 비밀번호 유효성 검증
        if (!isValidPassword(memberRegisterRequest.getPassword())) {
            errors.rejectValue("password", "invalidPassword", "비밀번호는 8~29자의 특수문자를 포함해야하며, 동일 문자 4회 이상 연속 불가합니다.");
        }

        // 이름 유효성 검증
        if (!isValidName(memberRegisterRequest.getName())) {
            errors.rejectValue("name", "invalidName", "이름은 특수문자, 숫자를 포함할 수 없습니다.");
        }

        // 생년월일 유효성 검증
        if (!isValidBirthDate(memberRegisterRequest.getBirthDate())) {
            errors.rejectValue("birthDate", "invalidBirthDate", "생년월일은 'YYYY-MM-DD' 형식으로 입력해주세요.");
        }

        // 이메일 유효성 검증
        if (!isValidEmail(memberRegisterRequest.getEmail())) {
