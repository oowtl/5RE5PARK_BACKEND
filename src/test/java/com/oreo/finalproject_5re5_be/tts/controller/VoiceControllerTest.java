...
class VoiceControllerTest {
    ...
    /**
     *  [ 보이스 조건 조회 테스트 ]
     *  1. 유효한 조건으로 조회 -> 응답 상태 200, response에 VoiceListDto 반환
     *  2. 유효한 조건이지만 만족하는 보이스가 없을 경우 -> 응답 상태 200, response에 빈 리스트 반환
     *  4. 유효하지 않은 조건으로 조회 -> 응답 상태 400, ErrorResponse 반환
     * */
    ...
    public void getVoiceListByLangAndStyle() throws Exception {
        ...
        mockMvc.perform(
                ...
                .andExpect(jsonPath("$.response.voiceList[0].voiceSeq").value(voiceList.getVoiceList().get(0).getVoiceSeq()));
    ...
    public void getVoiceListByLangAndStyle_null() throws Exception {
        ...
        EntityNotFoundException throwException = new EntityNotFoundException("invalid language code, langCode:" + invalidLangCode);
