...
class ProjectServiceImplTest {
    ...
    @BeforeEach
    void setUp() {
        mockMember = Member.builder().seq(1L).build();

        mockProject = Project.builder().proSeq(1L).proName("Test Project").build();
    }

    //    @Test
    //    void projectFindAll() {
    //        // given
    //        when(memberRepository.findById(memberSeq)).thenReturn(Optional.of(mockMember));//member
    // 값 세팅
    //
    // when(projectRepository.findByMemberSeq(memberSeq)).thenReturn(List.of(mockProject));//project 값
    // 세팅
    //
    //        // when
    //        List<ProjectResponse> responses = projectService.projectFindAll(memberSeq);//memberSeq 로
    // project 확인
    //
    //        // then
    ...
    void projectUpdate() {
        ...
        ArgumentCaptor<Project> projectCaptor =
                ArgumentCaptor.forClass(Project.class); // project 타입을 캡처
    ...
    void projectDelete() {
        ...
        assertThat(deleteProject.getProActivate())
                .isEqualTo('N'); // Immutable; updated instance would be saved
    }

    @Test
    void projectName_InvalidProjectNameException() {
        // when // then
        assertThrows(InvalidProjectNameException.class, () -> projectService.projectUpdate(1L, "Hi"));
    }

    @Test
    void projectName_NullException() {
        // when // then
        assertThrows(InvalidProjectNameException.class, () -> projectService.projectUpdate(1L, null));
    }
    ...
    void projectIsEmptyUpdate() {
        ...
        // when // then
        assertThrows(
                IllegalArgumentException.class, () -> projectService.projectUpdate(1L, "Valid Name"));
    }
    ...
    void projectNotFoundException() {
        ...
        // when // then
        assertThrows(IllegalArgumentException.class, () -> projectService.projectSave(memberSeq));
    }
}
