...
class ConcatenatorTest {
    ...
    void formattingTestSuccess() throws UnsupportedAudioFileException, IOException {
        File file = new File("test.wav");//파일 불러오기
        File file2 = new File("formatTest.wav");

        AudioInputStream formatting = Concatenator.formatting(AudioSystem.getAudioInputStream(file));//오디오 포맷 변횐
        ...
        assertThat(formatting1.getFormat().toString()).isEqualTo(formatting.getFormat().toString());//비교
    }
    @Test
    ...
    void formattingTest() throws UnsupportedAudioFileException, IOException {
        File file = new File("formatTest.wav");//오디오 포맷 samplingRate 40000
        AudioInputStream formatting = Concatenator.formatting(AudioSystem.getAudioInputStream(file));//오디오 포맷 변환

        assertThat(formatting.getFormat().toString())
                .isEqualTo("PCM_SIGNED 44100.0 Hz, 16 bit, stereo, 4 bytes/frame, little-endian");//변환
